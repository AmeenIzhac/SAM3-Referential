#!/usr/bin/env python3
"""Run LISA-7B over MMR val and score gIoU/cIoU.

Adapted verbatim from run_m2sa_mmr.py: M2SA is a fork of LISA and the two share
an identical model_forward/evaluate interface, so both models are driven through
exactly the same protocol, prompts and meters -- the only differences are the
model class and the weights.
...
Must run with the m2sa conda env python (same stack: py3.8/torch2.1/tf4.31),
cwd = the LISA repo.
"""
...
sys.path.insert(0, "/tmp/claude-1009/-workspace-reasonseg/d6cd4b24-35e9-4fa3-8cad-354194dce19a/scratchpad/LISA")
...
MODEL = "/mnt/data0/ameen/models/LISA-7B-v1"
IMAGE_TOKEN = "<image>"
...
def build(device, dtype):
    ...
    # M2SA hard-asserts 32003; LISA-7B-v1 ships its own tokenizer, so just report
    # the resolved id rather than assuming the two vocabularies line up.
    print(f"[lisa] vocab={len(tok)} seg_token_idx={seg_idx}", flush=True)
    return tok, model
...
def main():
    ...
    for ri, (rec_idx, rec) in enumerate(sharded, 1):
        ...
        if ri % 25 == 0:
            el = time.time() - t0
            print(f"[lisa-{args.mode} s{args.shard}] {ri}/{len(sharded)} imgs, {nq} q, "
